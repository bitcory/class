...
강사 대본(멘트·시간·팁)은 **수강생에게 보이면 안 되는 내용**이라 비밀 암호 링크로만 열립니다.

- 강사용 링크: `.../week/1?teacher=ssam2026`  (열면 대본이 켜지고 이 브라우저에 저장됨)
- 끄기: `.../week/1?teacher=off`
- 암호는 `src/lib/prefs.tsx` 의 `TEACHER_PASS` 값. 바꾸려면 이 값만 수정.
- 수강생 화면엔 강사 모드 버튼 자체가 없고, `?teacher=1`(옛 방식)은 막혀 있음.
- 주의: 정적 사이트라 대본 텍스트는 기술적으로 번들 안에 있음. 개발자 도구로 파고들면
  볼 수는 있으나, 일반 수강생에겐 충분한 수준의 가림.
