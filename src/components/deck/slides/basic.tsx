        ...
        {slide.pairs.map((p, i) => (
          <div key={i} className="rounded-2xl border-2 border-border p-5 sm:p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <span className="shrink-0 whitespace-nowrap rounded-xl bg-foreground px-3 py-2 text-center text-xl font-black text-background sm:text-2xl">
                {p.term === "한식·중식·일식" ? (
                  <MultiHighlightText
                    text={p.term}
                    matches={[
                      { match: "한식", className: "text-white-500" },
                      { match: "중식", className: "text-white-500" },
                      { match: "일식", className: "text-white-500" },
                    ]}
                  />
                ) : (
                  p.term
